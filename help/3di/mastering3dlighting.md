...
![壁にネオンサインが付いた駐車場内の車の3D CGIシーン 照明が、昼光からネオンLEDに変わります](assets/Mastering3dlighting_1.gif)
...
![セット全体（ライトを含む）が3Dに組み立てられ、スタジオのような結果が得られたシーンの例](assets/Mastering3dlighting_6.jpg)
...
![スタジオの3Dシーンを正距円筒図法の32ビット画像にレンダリングして、3Dスタジオ環境光を作成します](assets/Mastering3dlighting_8.png)

現実世界の写真を撮影して環境光を作成することもできます。 このワークフローには、360カメラが必要です（例： [Ricoh Theta Z1](https://theta360.com/en/about/theta/z1.html)）。 次に、カメラは露光量ブラケティングまたは同じ環境の複数のショットに使用され、一定の範囲の異なる露光値（露光量不足から露光量過多まで）で撮影されます。 これらのショットは、32ビット画像を作成するために使用され、通常はHDR（ハイダイナミックレンジの略称）と呼ばれます。 このような画像を構成する1つの方法として、Photoshopの「HDRに統合」機能があります。 埋め込まれた露光量範囲が、強度プロパティになります。

![写真の環境光で照らされた3Dシーンの例](assets/Mastering3dlighting_9.jpg)

![写真の環境光は、露光量ブラケットを使用して作成し、PhotoshopでHDR Proに統合します](assets/Mastering3dlighting_10.jpg)

いずれの場合も、光源（およびその強度）はこれらの画像に「焼き付け」られ、[!DNL Dimension]で使用されると光を発します。

これらの方法では、必要な照明、反射、ディテールをすべてキャプチャしましたが、3Dアプリでは3D空間でこれらの編集を引き続き行えるので、照明の回転を調整したり、全体の強度やカラーを変更したりできます。

![3Dシーンで環境光の強さと方向を操作する](assets/Mastering3dlighting_11.gif)
...
[!DNL Dimension]には、「ライトをポイントに向ける」という便利なアクションがあります。このアクションでは、3Dオブジェクト上でクリックしてドラッグするだけで、回転と高さを制御できます。 これにより、光線の方向を動的に変更できます。 これらのパラメーターは手動で調整することもできます。

指向性ライトのカラーと強度を変更したり、光源の形状を調整したりできます（円形または長方形にしたり、伸ばしたり、大きくしたりします）。 最後に、光源のエッジをソフトにします。

![Adobe[!DNL Dimension]](assets/Mastering3dlighting_12.gif)の指向性ライトの形状を変更しています
...
![照明の強度、方向、サイズが3Dオブジェクトの照明にもたらす効果と、オブジェクトが投影する影を示す図](assets/Mastering3dlighting_13.jpg)
...
![Adobe [!DNL Dimension]](assets/Mastering3dlighting_15.gif)の3D自動車モデルで太陽光の照明プロパティを操作

![太陽光だけが光る月の3Dシーン](assets/Mastering3dlighting_16.jpg)

環境光を使って空を再現できます。また、空の特徴を生かした環境光も使用できます。 次に、[!DNL Dimension]で作成された太陽光を、環境光で撮影された太陽に合わせる必要があります。 これは、球体を作成し、その球体にメタルのマテリアルを割り当てることで簡単におこなうことができます。これにより、環境の反射がリアルタイムで得られるので、「ライトをポイントに向ける」を使用して、太陽光と太陽の位置合わせることができます。

曇り空の特徴を生かした環境光を使用する場合は、曇り具合のプロパティを使用して、これらの条件により近い状態にできます。

![Adobe [!DNL Dimension]](assets/Mastering3dlighting_17.gif)の3D自動車モデルで空の環境光の曇りのプロパティを操作しています

太陽光と空の環境光を組み合わせたら、グローバル回転プロパティを使用してこれらを同時に回転できます。

### オブジェクトベースのライト

マテリアルのGlowプロパティをオンにすると、オブジェクトを光源にすることができます。 これにより、電球、ネオンライト、ソフトボックス、あらゆる種類の画面やディスプレイなどのオブジェクトを作成できます。

このタイプの照明を使用する主な利点は、照度の減衰です。これにより、非常に自然な結果が得られます。 これは、製品のビジュアル化や他のスタジオベースのシーンで非常に便利です。

![減衰（発光プレート）を持つ光源と無限の光源（指向性ライト）の比較](assets/Mastering3dlighting_18.png)

変形ツールを使用して、発光オブジェクトを拡大または縮小することで、シャドウの柔らかさを制御できます。 大きくすると、光の強さも強くなります。

![オブジェクトの光のサイズを変更すると、光の量が増え、影がソフトになります](assets/Mastering3dlighting_19.gif)

以前に説明したタイプのライトとは異なり、これらのライトでは、単色に加えてテクスチャも使用できます。 テクスチャはマテリアルのベースカラーにアタッチでき、光の強さは発光スライダーで制御します。

![3D自動車モデルを照らすオブジェクトライトにテクスチャを適用しています](assets/Mastering3dlighting_20.gif)

## 効果的な3D照明の例

### 製品の照明

![3Dヘッドホンモデルを照らす3つのライト（キー、フィル、リム）を個別に照らした場合と3つのライトが同時に動作した場合の例](assets/Mastering3dlighting_21.gif)

プロダクトショットの照明を設定するには、多くの撮影技法があります。 ここでは、最も一般的に使用される設定の1つである3点ライトシステムを使用します。

この設定は、次の3つのライトで構成されます。

1. **主光源として使用する**&#x200B;キーライト。ほぼカメラの方向から光が当たります。

   ![3Dヘッドホンモデルを照らすキーライトの例](assets/Mastering3dlighting_22.jpg)

1. **リムライト:**&#x200B;キーとは反対側の方向から照らして、被写体のシルエットを見せるために使用します。

   ![3Dヘッドホンモデルを照らすリムライトの例](assets/Mastering3dlighting_23.jpg)

1. **フィルライト:**&#x200B;比較的弱い光で、暗い部分を補う役割をします。これは、前の2つのライトが届かない部分に使用されます。

   ![3Dヘッドホンモデルを照らすフィルライトの例](assets/Mastering3dlighting_24.jpg)

[!DNL Dimension]で3点ライトを作成する方法は2つあります。 1つは、指向性ライトを使用する方法（シーンに個別に追加する、または3点ライトのプリセットを使用する）、もう1つは発光オブジェクトを使用する方法です。
...
![Pipe Dreams by Vladimir Petkovicという3Dアートワーク](assets/Mastering3dlighting_27.jpg)
...
![ウラジミール・ペトコビッチの『パイプ・ドリームズ』の3D照明設定を示す図](assets/Mastering3dlighting_28.jpg)
...
![Adobe[!DNL Dimension]のグローバルな回転を使用して、3Dシーンで太陽と空のライトシステムの向きを変更する](assets/Mastering3dlighting_34.gif)

この概要で、様々な状況で3Dライトをマスターすることの重要性を説明したいと思います。 より説得力のある結果を生み出す準備が整っているはずです。

素敵な照明を使用しましょう。 Dimensionの[最新リリース](https://creativecloud.adobe.com/apps/download/Dimension)を今すぐダウンロードしてください。
