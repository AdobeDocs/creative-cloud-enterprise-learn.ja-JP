---
title: 生成再配色
description: プロジェクトの様々なカラーバリエーションを即座にプレビューする方法を説明します
role: User
level: Beginner
jira: KT-15040
exl-id: 2b47eeb6-0ce2-4fe8-89b7-feca4f264174
TQID: https://experienceleague.adobe.com/99Z4SlCxNnYwr-sLImNyzgvSiDtSFqg4ShLUA0yJ-Jk
product_v2: id: e66c61b1-1ca4-4c42-8df9-e5cb44b0555c
feature_v2: id: fec89bf3-1b77-4b07-a0b9-96726856a0ad
subfeature_v2: id: aaae4770-bc47-47c2-876b-1fbcb533c42aid: b29e1156-4668-4c0c-84e3-9347e94225edid: d1878b8b-dcd8-4fb4-9ec7-8030a8c54669
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2: id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
source-git-commit: af4fdb8dfc8c84e5e51525c730d0ba8831179443
workflow-type: tm+mt
source-wordcount: 70
ht-degree: 2%

---

# 生成的再配色

ベクターファイルで、製品のパッケージ、ポスター、アートワークの様々なカラーバリエーションを簡単にプレビューする方法を説明します。 生成再配色を使用すると、入力できるだけすばやく再配色を行うことができます。 壁から離れたアイデアを試し、様々なテーマやパレットを中心にアイデアを思い描いて、複数のバリエーションを瞬時に作成できます。 [こちら](https://firefly.adobe.com/?locale=jp)でAdobe Fireflyを試してください。

<br> 

>[!VIDEO](https://video.tv.adobe.com/v/3427610?quality=12&learn=on&hidetitle=true)
